...
_KONFI		bit 7Bh
_SKLA		equ 7Ch		;stan klawiatury
_OKLA		equ 7Dh		;poprzedni stan klawiatury
...
_KLAWIATURA:
	...
	MOV R7,#1		;wybieram najmlodszy bit do wyboru klawiszy
	MOV _SKLA,#0		;zeruje stan poczatkowy klawiatury
_KL_FOR:

	MOV A , R7		;aktualnie wybrany klawisz patrze
	MOV DPTR,#_CSDS		;czy jest nacisniety
	MOVX @DPTR,A		;-//-
	MOV C,P3.5		;-//-
	JNC _POMIN
	ORL _SKLA,A		;jesli jest to zapisuje go
_POMIN:
	RL A			;wybor nastepnego klawisza
	MOV R7,A		;zapisuje wybor w R7

	JNB ACC.6,_KL_FOR	;sprawdzam czy sprawidzilem wszystkie klawisze

	MOV A,_SKLA
	CJNE A,_OKLA,_KLAWIATURA_DZI
	RET

_KLAWIATURA_DZI:		;patrze co zostalo wcisniete
	MOV _OKLA,A
	JB ACC.0,_ENTER
	JB ACC.3,_UP
	JB ACC.4,_DOWN
